...
describe("Formulario", () => {
  ...
  test("Se os campos estiverem vazios, o botão deve estar desabilitado", () => {
    const { getByPlaceholderText, getByRole } = render(
      <Formulario aoSubmeter={mockAoSubmeter} />
    );

    const inputNome = getByPlaceholderText("Insira o nome do filme");
    const inputAnoDeLancamento = getByPlaceholderText(
  ...
  test("Deve chamar a função aoSubmeter com os valores corretos", async () => {
    const { getByPlaceholderText, getByRole } = render(
      <Formulario aoSubmeter={mockAoSubmeter} />
    );
    ...
    await userEvent.click(botaoAdicionar);
    expect(mockAoSubmeter).toHaveBeenCalledWith({
      nome: "Filme",
      anoDeLancamento: "2021",
    });
  });
  test("Deve limpar os campos após submeter", async () => {
    const { getByPlaceholderText, getByRole } = render(
      <Formulario aoSubmeter={mockAoSubmeter} />
    );
    ...
    expect(inputAnoDeLancamento).toHaveValue("");
  });
});
